const gulp = require('gulp');
const sass = require('gulp-sass');
const rename = require('gulp-rename');
const concat = require('gulp-concat');
const minifyCSS = require('gulp-clean-css');
const autoprefixer = require('gulp-autoprefixer');
const plumber = require('gulp-plumber');
const sourcemaps = require('gulp-sourcemaps');
const changed = require('gulp-changed');
const prettify = require('gulp-prettify');
const pug = require('gulp-pug');
const browserSync = require('browser-sync').create();
const babel = require('gulp-babel');
const javascriptObfuscator = require('gulp-javascript-obfuscator');

const src = './source';
const pub = './public';
const fontAwesome = 'node_modules/font-awesome'

// VARIABLES
const paths = {
  json: {
    src: `${src}/content/config/*.json`,
    dest: `${pub}/content/config`,
  },
  cssVendor: {
    src: [`${src}/assets/css/**/*.css`],
    dest: `${pub}/assets/css`,
  },
  sass: {
    src: [`${src}/assets/scss/main.scss`],
    inc: `${src}/assets/scss/**/*.{scss,sass}`,
    dest: `${pub}/assets/css`,
  },
  pugIndex: {
    src: `${src}/pug/index.pug`,
    dest: `${pub}/`,
  },
  pugIndexUT: {
    src: `${src}/pug/content/*.pug`,
    inc: `${src}/pug/content/includes/**/*.{pug,html}`,
    dest: `${pub}/content/`,
  },
  pugPages: {
    src: `${src}/pug/content/pages/*.pug`,
    inc: `${src}/pug/content/pages/includes/**/*.{pug,html}`,
    dest: `${pub}/content/pages`,
  },
  jsGlobal: {
    src: [
      `${src}/content/js/global/router.js`,
      `${src}/content/js/global/app.js`,
    ],
    fileName: "global.js",
    dest: `${pub}/content/js`,
  },
  jsVendor: {
    src: [
      `${src}/assets/vendor/libs/jquery-3.4.1.min.js`,
      `${src}/assets/vendor/libs/bootstrap.bundle.min.js`,
      `${src}/assets/vendor/libs/jquery.loader.min.js`,
      `${src}/assets/vendor/libs/jquery.router.min.js`,
      `${src}/assets/vendor/libs/jquery.fancybox.min.js`,
      `${src}/assets/vendor/libs/wow.min.js`,
      `${src}/assets/vendor/plugins/*.js`,
    ],
    fileName: "plugins.js",
    dest: `${pub}/assets/js`,
  },
  jsContent: {
    src: `${src}/content/js/pages/*.js`,
    fileName: "pages.js",
    dest: `${pub}/content/js`,
  },
  media: {
    src: [
      `${src}/assets/fonts/**/*.*`,
      `${src}/assets/images/**/*.*`,
      `${src}/content/download/**/*.*`,
      `${src}/content/images/**/*.*`,
      `${src}/content/media/**/*.*`,
    ],
  },
  vendor: {
    jquery: "./node_modules/jquery/dist/jquery.min.js",
    bootstrap: "./node_modules/bootstrap/dist/js/bootstrap.bundle.min.js",
    popper: "./node_modules/popper.js/dist/popper.min.js",
    fontawesome: {
      src: [
        `node_modules/font-awesome/css/**/*.*`,
        `node_modules/font-awesome/fonts/**/*.*`,
      ],
      dest: `${pub}/assets/fonts/fontawesome/`,
    },
  },
};

// COMPILAR PUG
gulp.task('pugIndex', done => {
  gulp
    .src(paths.pugIndex.src)
    // PREVIENE QUE LOS PROCESOS GULP.WATCH SE DETENGA AL ENCONTRAR UN ERROR
    .pipe(plumber())
    // SE ENCARGA DE QUE SOLO COMPILE EL ARCHIVO QUE CAMBIO
    .pipe(changed(paths.pugIndex.dest, { extension: '.html' }))
    // COMPLIA PUG
    .pipe(
      pug({
        locals: {}
      })
    )
    // ENBELLECE EL HTML
    .pipe(prettify({ indent_size: 4 }))
    // GUARDA EL ARCHIVO HTML
    .pipe(gulp.dest(paths.pugIndex.dest))
    // REFRESCADO DEL NAVEGADOR
    .pipe(browserSync.stream());

  done();
});

// COMPILAR PUG
gulp.task('pugIndexUT', done => {
  gulp
    .src(paths.pugIndexUT.src)
    // PREVIENE QUE LOS PROCESOS GULP.WATCH SE DETENGA AL ENCONTRAR UN ERROR
    .pipe(plumber())
    // COMPLIA PUG
    .pipe(
      pug({
        locals: {}
      })
    )
    // ENBELLECE EL HTML
    .pipe(prettify({ indent_size: 4 }))
    // GUARDA EL ARCHIVO HTML
    .pipe(gulp.dest(paths.pugIndexUT.dest))
    // REFRESCADO DEL NAVEGADOR
    .pipe(browserSync.stream());
  done();
});

gulp.task('pugPages', done => {
  gulp
    .src(paths.pugPages.src)
    // PREVIENE QUE LOS PROCESOS GULP.WATCH SE DETENGA AL ENCONTRAR UN ERROR
    .pipe(plumber())

    // COMPLIA PUG
    .pipe(
      pug({
        locals: {}
      })
    )

    // ENBELLECE EL HTML
    .pipe(prettify({ indent_size: 4 }))

    // GUARDA EL ARCHIVO HTML
    .pipe(gulp.dest(paths.pugPages.dest))

    // REFRESCADO DEL NAVEGADOR
    .pipe(browserSync.stream());

  done();
});

gulp.task('sass', done => {
  gulp
    .src(paths.sass.src)
    .pipe(plumber())
    .pipe(sourcemaps.init())
    .pipe(autoprefixer())
    .pipe(sass({ outputStyle: 'compressed' }).on('error', sass.logError))
    .pipe(sourcemaps.write('.'))
    .pipe(gulp.dest(paths.sass.dest))
    //.pipe(rename({ suffix: '.min' }))
    //.pipe(minifyCSS())
    //.pipe(gulp.dest(paths.sass.dest))
    .pipe(browserSync.stream());
  done();
});

gulp.task('font-awesome', done => {
  gulp
    .src(paths.vendor.fontawesome.src, { base: fontAwesome })
    .pipe(gulp.dest(paths.vendor.fontawesome.dest));
  done();
})

gulp.task("cssVendor", (done) => {
  gulp.src(paths.cssVendor.src).pipe(gulp.dest(paths.cssVendor.dest));
  done();
});

gulp.task('json', done => {
  gulp
    .src(paths.json.src)
    .pipe(gulp.dest(paths.json.dest))
    .pipe(browserSync.stream());
  done();
})

/* Archivos Imágenes y documentos usados en cada tema. */
gulp.task('media', done => {
  paths.media.src.forEach(route => {
    gulp
      .src(route, { base: src })
      .pipe(gulp.dest(pub))
  })
  done();
});

/* Librerias Javascript que se utilizan en todo el sitio */
gulp.task('jsVendor', done => {
  gulp
    .src(paths.jsVendor.src)
    .pipe(concat(paths.jsVendor.fileName))
    .pipe(gulp.dest(paths.jsVendor.dest))
    .pipe(browserSync.stream());
  done();
});

gulp.task('jsGlobal', done => {
  gulp
    .src(paths.jsGlobal.src)
    .pipe(concat(paths.jsGlobal.fileName))
    .pipe(gulp.dest(paths.jsGlobal.dest))
    .pipe(browserSync.stream())
  done();
})

/* Scripts task */
gulp.task('jsContent', done => {
  gulp
    .src(paths.jsContent.src)
    .pipe(concat(paths.jsContent.fileName))
    .pipe(gulp.dest(paths.jsContent.dest))
    .pipe(browserSync.stream());
  done();
});

gulp.task('servidor', done => {
  browserSync.init({
    server: pub,
    open: true,
    notify: true
  });
  done();
});


gulp.task('watch', done => {
  gulp.watch(paths.pugIndex.src, gulp.series('pugIndex'))
  gulp.watch([paths.pugIndexUT.src, paths.pugIndexUT.inc], gulp.series('pugIndexUT'));
  gulp.watch([paths.pugPages.src, paths.pugPages.inc], gulp.series('pugPages'));
  gulp.watch(paths.sass.inc, gulp.series('sass'));
  gulp.watch(paths.cssVendor.src, gulp.series("cssVendor"));
  gulp.watch(paths.json.src, gulp.series('json'))
  gulp.watch(paths.media.src, gulp.series('media'))
  gulp.watch(paths.jsVendor.src, gulp.series('jsVendor'))
  gulp.watch(paths.jsContent.src, gulp.series('jsContent'))

  gulp.watch(paths.jsGlobal.src, gulp.series('jsGlobal'))

  done();
});

gulp.task('default',
  gulp.series(
    'json',
    'media',
    'font-awesome',
    'jsVendor',
    'jsGlobal',
    'jsContent',
    'pugIndex',
    'pugIndexUT',
    'pugPages',
    'sass',
    'cssVendor',
    'servidor',
    'watch'
  )
);
